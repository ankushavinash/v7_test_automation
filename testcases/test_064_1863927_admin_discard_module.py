...
class Test_064:
    ...
    def test_064_admin_discard_module(self, setup):
        self.logger.info("********test_064_admin_discard_module : started********")
        ...
        # admin discard module
        generic_comment = "testing generic comment"
        rp.admin_discard_module(generic_comment, main_window)
        ...
        # driver.find_element_by_id("TransitionId_5159").click()
        # # generic comment
        # driver.find_element_by_id("H13881").send_keys(generic_comment)
        # rp.click_ok()

        # validation
        if bu.is_displayed((By.ID, "F11197")):
            text = bu.get_text((By.ID, "F11197"))
            self.logger.info("******discard module as admin user successful. status : " + text + "*******")
            assert True, "discard module as admin user successful. status : " + text
        else:
            self.logger.info("******discard module as admin user unsuccessful*******")
            assert False, "discard module as admin user unsuccessful."

        self.logger.info("********test_064_admin_discard_module  : passed*******")
        self.logger.info("*********test_064_admin_discard_module  : completed ********")
